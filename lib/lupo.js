...
(function (window, document, $, undefined) {
    ...
    function doWatch(scope, prop, func) {
        ...
        modelCopy[scope.template] = JSON.stringify(scope.model);
        // watch properties
        watch(scope, [prop], function (prop, action, newvalue, oldvalue) {
            ...
            watchTimer = window.setTimeout(function () {
                ...
                // if push/pop/splice/sort on model lists.
                if (action === 'push' || action === 'pop' || action === 'splice' || action === 'sort') {

                    if (modelCopy[scope.template] === JSON.stringify(scope.model)) {
            ...
            }, 50);  // delay so func() wont be called once for each updated element, just once for all. i.e. on keyup.
        });
    }
