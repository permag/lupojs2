...
(function () {
    ...
    function modelItemToView (compOb, currKey) {
        ...
        if (compOb.model && currKey) {
            ...
            compOb.viewEl.find('[data-model="' + currKey + '"]').val(value).html(value);
        }
    }
    ...
    lupo.router = function (routes) {
        ...
        function router() {
            var hash = window.location.hash, route, func, hashSplit, routeSplit, i, len = 0, params = [];
            for (route in routes) {
                if (!routes.hasOwnProperty(route) && typeof routes[route] !== 'function') break;
                func = routes[route];
                hashSplit = hash.substring(hash.indexOf('/') + 1).split('/');
                routeSplit = route.substring(route.indexOf('/') + 1).split('/');
                len = hashSplit.length;
                if (len !== routeSplit.length) continue;
                for (i = 0; i < len; i += 1) {
                    if (routeSplit[i] !== hashSplit[i] && routeSplit[i].charAt(0) !== ':') break;
                    if (routeSplit[i].charAt(0) === ':') params.push(hashSplit[i]);
