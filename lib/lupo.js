...
(function (window, document, $, undefined) {
    ...
    function run(compOb) {
        ...
        if (!watchObject[compOb.template]) {
            doWatch(compOb, 'model', modelAllToView);
            doWatch(compOb, 'form', formAllToView);
            watchObject[compOb.template] = true;  // mark these object watched, by its template.
        }
    }
    ...
    function doWatch(compOb, prop, func) {
        ...
        // watch properties
        watch(compOb, [prop], function (prop, action, newvalue, oldvalue) {
            ...
            watchTimer = window.setTimeout(function () {
                ...
                if (action === 'push' || action === 'pop') {  // if push/pop to model lists.
                    renderCache[compOb.template] = null;  // clear cache
                    if (compOb.template === currentTemplate) {
                        lupo.render(compOb);  // only re-render view if it's current one.
                    }
                } else {
                    func(compOb);
                }
        ...
        $.each(compOb.events, function (key, eventFn) {
            ...
            $(evEl[1]).bind(evEl[0], function (e) {
                ...
                if (typeof fn === 'function') {
                    fn.call(compOb, e);  // call function name
                } else if (typeof eventFn === 'function') {
                    eventFn.call(compOb, e);  // call anonymous function expression
                }
            });
        });
    }
    ...
    lupo.defineComponent = function (compName, fn) {
        ...
        lupo.component[compName] = function () {
            var customReturns = fn.call(this, scope),
                ret = {};
            // if no return in comp, return render method.
            if (typeof customReturns === 'undefined' || !customReturns) {
                ret = {render: scope.render};
            } else {
                // if comp has return, but not returning render method,
                // add render method to return.
                if (typeof customReturns.render === 'undefined') {
                    ret = customReturns;
                    ret.render = scope.render;
                }
                // comp has return and is returning render method.
                ret = customReturns;
            }
            return ret;
        };
