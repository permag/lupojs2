...
(function () {
    ...
    function watchModel(compOb) {
        var keys = [];
        $.each(compOb.model, function (key) {
            keys.push(key);
        });
        watch(compOb.model, keys, function () {
            modelAllToView(compOb);
        });
    }
    ...
    function dataForm(compOb) {
        var view = $('[data-view="' + compOb.view + '"]'),
            formElem, currProp;
        $.each(view.find('*'), function (key, elem) {
            if ($(elem).attr('data-form')) {
                formElem = $(elem);
                currProp = getPropertyValueByPath(compOb.form, formElem.attr('data-form'));
                formElem.attr('disabled', currProp.disabled);
                formElem.val(currProp.text).html(currProp.text);
            }
        });
    }
    ...
    function modelAllToView(compOb) {
        ...
        $.each(view.find('*'), function (key, elem) {
            if ($(elem).attr('data-model')) {
                obPath = $(elem).attr('data-model');
                obVal = getPropertyValueByPath(compOb.model, obPath);
                $(elem).val(obVal).html(obVal);
            }
        });
    }

    function viewToModel (compOb) {
        var view = $('[data-view="' + compOb.view + '"]'),
            target, currKey, newValue;
        view.find('input, textarea').bind('blur', function (e) {
            e.preventDefault();
            target = $(e.target);
    ...
    lupo.render = function (compOb, data) {
        ...
        $.ajax({
            ...
            rendered = lupoTemplate(template, {model: model})
